class WaitListEntriesController < ApplicationController
  ...
  def create
    ...
    @waitlist_entry.position = @event.wait_list_entries.count + 1
    if @waitlist_entry.save
      ...
      redirect_to @event, notice: "You have been added to the waitlist at position #{@waitlist_entry.position}."
    else
  ...
  def destroy
    ...
    authorize @waitlist_entry
    if @waitlist_entry.destroy
      redirect_to @waitlist_entry.event, notice: "You have left the waitlist."
    else
      redirect_to @waitlist_entry.event, alert: @waitlist_entry.errors.full_messages.to_sentence
    end
  end
end
